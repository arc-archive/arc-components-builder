...
class ProjectOptions {
  constructor(opts) {
    ...
    /**
     * By default it creates a separate React file for each web component that
     * is to be wrapped in React code. When this option is set then it bundles
     * all React components into a single file and exports each React component
     * in this file.
     *
     ...
     * @type {Boolean}
     */
    this.bundleReactComponents = opts.bundleReactComponents;
    /**
  ...
  _validateReactOptions(userOpts) {
    ...
    if (userOpts.bundleReactComponents && !userOpts.react) {
      let message = '"bundleReactComponents" option given but "react" is not set. ';
      message += 'This option will be ignored.';
      this.validationWarnings.push(message);
      delete userOpts.bundleReactComponents;
    }
  }
